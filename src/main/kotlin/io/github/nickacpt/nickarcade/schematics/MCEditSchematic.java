...
public class MCEditSchematic implements ISchematic {
    ...
    // https://github.com/EngineHub/WorldEdit/blob/version/5
    // .x/src/main/java/com/sk89q/worldedit/schematic/MCEditSchematicFormat.java
    ...
    private ErrorMessage readBlocksData(NBTCompound nbtTag) {
        ...
        for (int index = 0; index < this.blockId.length; index++) {
            if ((index >> 1) >= this.addId.length) {
                ...
                if ((index & 1) == 0) {
                    this.blocks[index] =
                            (short) (((this.addId[index >> 1] & 0x0F) << 8) + (this.blockId[index] & 0xFF));
                } else {
                    this.blocks[index] =
                            (short) (((this.addId[index >> 1] & 0xF0) << 4) + (this.blockId[index] & 0xFF));
                }
            }
        }
